...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let destination_path = fs::read_link(format!("/proc/{pid}/fd/{fd}")).ok()?;
        let file_name = OpenFile::path_to_name(destination_path.to_str()?); // 转换为 String
        let contents = fs::read_to_string(format!("/proc/{pid}/fdinfo/{fd}")).ok()?; // 读取目标路径的内容
        
        match (OpenFile::parse_cursor(&contents), OpenFile::parse_access_mode(&contents)) {
            (Some(cursor), Some(access_mode)) => {
                Some(OpenFile {
                    name: file_name,
                    cursor: cursor,
